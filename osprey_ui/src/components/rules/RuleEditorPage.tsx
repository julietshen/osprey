...
const RuleEditorView: React.FC<{ data: BootstrapData }> = ({ data }) => {
  ...
  const [wireIntoMain, setWireIntoMain] = React.useState<boolean>(data.isNewRule);
  ...
  const canSubmit =
    !!validation?.ok && !!ruleNameForSubmit && submitState.kind !== 'submitting' && !!effectiveSource.trim();
          ...
          <div className={styles.headerActions}>
            ...
            >
              <Segmented
                value={mode}
                onChange={(value) => {
                  return setMode(value as EditorMode);
                }}
                options={[
