    ...
    def from_env(cls) -> 'GitLabConfig':
        ...
        if not project:
            raise RuleDraftBackendError(
                ...
                '(e.g. "julietshen/osprey-proof-of-concept").',
                status_code=503,
            )
        if not token:
            raise RuleDraftBackendError(
...
def _post(cfg: GitLabConfig, url: str, json: dict[str, Any] | None = None, **kwargs: Any) -> requests.Response:
    return requests.post(url, headers=_headers(cfg), json=json, timeout=DEFAULT_TIMEOUT_SECONDS, **kwargs)
...
class GitLabBackend:
    ...
    def submit_draft(
        ...
        *,
        ...
        branch = _generate_branch_name(rule_name, author_email)
