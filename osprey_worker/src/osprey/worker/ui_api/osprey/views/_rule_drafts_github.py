...
def load_config() -> GitHubConfig:
    ...
    return GitHubConfig(repo=repo, base_branch=base, rules_path=rules_path, token=token)
...
def _get(cfg: GitHubConfig, url: str, **kwargs: Any) -> requests.Response:
    return requests.get(url, headers=_headers(cfg), timeout=DEFAULT_TIMEOUT_SECONDS, **kwargs)


def _post(cfg: GitHubConfig, url: str, json: dict[str, Any]) -> requests.Response:
    return requests.post(url, headers=_headers(cfg), json=json, timeout=DEFAULT_TIMEOUT_SECONDS)


def _put(cfg: GitHubConfig, url: str, json: dict[str, Any]) -> requests.Response:
    return requests.put(url, headers=_headers(cfg), json=json, timeout=DEFAULT_TIMEOUT_SECONDS)
...
def _get_file_sha(cfg: GitHubConfig, path_in_repo: str, ref: str) -> str | None:
    """Return the blob sha for a file on `ref`, or None if it does not exist."""
...
def _require_already_present(main_sml: str, draft_path: str) -> bool:
    pattern = re.compile(
...
def _append_require_to_main(main_sml: str, draft_path: str) -> str:
...
def submit_draft(
    ...
    *,
    ...
    if wire_into_main:
        ...
        if not _require_already_present(main_contents, draft_path):
            new_main = _append_require_to_main(main_contents, draft_path)
            _commit_file(
                cfg,
                branch=branch,
...
def fetch_file_contents(cfg: GitHubConfig, draft_path: str, ref: str | None = None) -> str | None:
    path_in_repo = _full_path(cfg, draft_path)
    target_ref = ref or cfg.base_branch
    res = _get(
        cfg,
        f'{cfg.repo_url}/contents/{path_in_repo}',
        params={'ref': target_ref},
        headers={**_headers(cfg), 'Accept': 'application/vnd.github.raw'},
    )
    if res.status_code == 404:
        return None
    _raise_for_github(res, f'reading {path_in_repo!r} on {target_ref!r}')
    return res.text


def list_open_pull_requests(cfg: GitHubConfig) -> list[dict[str, Any]]:
    res = _get(
        cfg,
        f'{cfg.repo_url}/pulls',
