    ...
    def from_env(cls) -> 'TangledConfig':
        ...
        if not repo or '/' not in repo:
            raise RuleDraftBackendError(
                ...
                '(e.g. "julietshen.bsky.social/osprey-experiment").',
                status_code=503,
            )
        if not repo_did.startswith('did:'):
            raise RuleDraftBackendError(
                "OSPREY_TANGLED_REPO_DID is not configured; set it to the repo's DID "
                '(e.g. "did:plc:55uxnkmuoiwwgyoqjbjo4z5t"). Tangled shows this in the empty-repo '
...
def _generate_source_branch_name(rule_name: str, author_email: str) -> str:
    """Cosmetic source-branch label for the pull record. Tangled displays this
    but doesn't require it to correspond to a real git ref on the knot."""
    ...
    return f'osprey-ui/{slug}/{rule_slug}-{int(time.time())}'
...
class TangledBackend:
    ...
    def submit_draft(
        ...
        *,
        ...
        source_branch = _generate_source_branch_name(rule_name, author_email)
